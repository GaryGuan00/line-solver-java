...
public class CACHE {
    ...
    public static cacheMVAReturn cache_mva(Matrix gamma, Matrix m){
        ...
        for(int s = 0; s < SS.getNumRows(); s++){
            ...
            for(int l = 0; l < h; l++){
                Matrix mcur_l = UTIL.oner(mcur, new ArrayList<>(Collections.singletonList(l)));
                int s_l = UTIL.matchrow(SS, mcur_l);
                if(s_l >= 0){
    ...
    public static cacheXiFpReturn cache_xi_fp(Matrix gamma, Matrix m, Matrix xi){
        ...
        for(; it < 1.0e4; it++){
            ...
            for(int i = 0; i < n; i++){
                pi0.set(0, i, UTIL.max(tol, 1 - pij.sumRows(i)));
            }
            double DELTA = 0;
            for(int i = 0; i < n; i++){
