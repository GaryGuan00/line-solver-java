...
public class SolverLN extends EnsembleSolver {
    ...
    private SolverFactory solverFactory = SolverFactory.SolverMVA;
    ...
    public SolverLN(LayeredNetwork lqnmodel){
        super(lqnmodel,"SolverLN");
        constructSolverLN(lqnmodel);
        for(int i=0;i<nlayers;i++){
            solvers[i]= new SolverMVA(ensemble[i]);
        }
    }
    ...
    public SolverLN(LayeredNetwork lqnmodel, SolverFactory solverFactory){
        super(lqnmodel,"SolverLN");
        constructSolverLN(lqnmodel);
        assignSolverFactory(solverFactory);
    }

    public SolverLN(LayeredNetwork lqnmodel, SolverOptions options){
        super(lqnmodel,"SolverLN",options);
        constructSolverLN(lqnmodel);
        for(int i=0;i<nlayers;i++){
            solvers[i]= new SolverMVA(ensemble[i]);
        }
    ...
    private void constructSolverLN (LayeredNetwork lqnmodel){
    ...
    private void assignSolverFactory(SolverFactory solverFactory){
        this.solverFactory=solverFactory;
        if(this.solverFactory== SolverFactory.SolverFluid) {
            for (int i = 0; i < nlayers; i++) {
                solvers[i] = new SolverFluid(ensemble[i]);
                solvers[i].options.stiff=false;
            }
        }else if(this.solverFactory== SolverFactory.SolverMVA){
            for (int i = 0; i < nlayers; i++) {
                solvers[i] = new SolverMVA(ensemble[i]);
                solvers[i].options.stiff=false;
            }
        }
        //todo add here if new solverFactory avaiable
    }
    ...
    public void iterate(){
        ...
        while (!converged(it) && it < options.iter_max) {
            ...
            if (options.verbose != SolverOptions.VerboseLevel.SILENT) {
                System.out.format(
                        "Analyze: %fs. Update: %fs. Runtime: %fs. \n",
    ...
    public void analyze(int it, int e) {
        ...
        result1.CN = solvers[e].result.CN;
        result1.XN = solvers[e].result.XN;

        if (results.containsKey(it)) {
            results.get(it).put(e, result1);
        } else {
            HashMap<Integer, SolverResult> modelIdxToResultMap = new HashMap<>();
            modelIdxToResultMap.put(e, result1);
            results.put(it, modelIdxToResultMap);
        }
    }

    public boolean converged(int it) {
        // task3: converged function to written
        ...
        boolean bool = false;
        //this.options.iter_max is 100 in MatLab, while set as 10 in SolverOptions.java
        this.options.iter_max = 100;
        int iter_min = Math.min(30, (int)Math.ceil(this.options.iter_max / 4.0));
        int E = this.nlayers;
            ...
            if (averagingstart != null) {
                ...
                if (it >= iter_min) {
                    for (int e = 0; e < E; e++) {
                        ...
                        results.get(results.size()).get(e).XN.add(mov_avg_weight - 1,
                                results.get(results.size()).get(e).XN);
...
//                                results.get(results.size()).get(e).WN);
                        ...
                        for (int k = 1; k < wnd_size; k++) {
                            ...
                            results.get(results.size()).get(e).XN.add(mov_avg_weight,
                                    results.get(results.size() - k).get(e).XN);
...
//                                    results.get(results.size() - k).get(e).WN);
        ...
        if(it > 1){
            ...
            for(int en = 0; en < E; en++){
                Matrix metric = results.get(results.size()).get(en).QN;
                Matrix metric_1 = results.get(results.size() -1).get(en).QN;
                int N = (int)this.ensemble[en].getNumberOfJobs().elementSum();
                if(N > 0){
                    int IterErr;
                    try {
                        ...
                        difference01.abs();
                        IterErr = (int)difference01.elementMax() / N;
                    }
                    catch (Exception e){
                        IterErr = 0;
                    }
            ...
            if(this.options.verbose != SolverOptions.VerboseLevel.SILENT){
                String msg = String.format("\bChange: %.2f.", this.maxitererr.get(it)/E);
                System.out.println(msg);
                if(it == iter_min){
                    System.out.println(" Starting averaging to help convergence.");
        ...
        if((it > 2) && (this.maxitererr.get(it) < this.options.iter_tol) && (this.maxitererr.get(it - 1)
    ...
    public void initFromRawAvgTables(){
        System.out.println("initFromRawAvgTables haven't be implemented in LINE");
    }
    ...
    public void init() {
        //task4: init function to be written
        ...
        if(this.route_prob_updmap.nz_length == 0){
            ...
            unique_route_prob_updmap = new Matrix(numSet.size(), numSet.size(), numSet.size() * numSet.size());
            for(int k = 0 ; k < numSet.size(); k++)
                this.unique_route_prob_updmap.set(k, numSet.get(k));
        }

        this.tput = new Matrix(1, this.lqn.nidx + 1, this.lqn.nidx);
        this.util = new Matrix(1, this.lqn.nidx + 1, this.lqn.nidx);
        this.servt = new Matrix(1, this.lqn.nidx + 1, this.lqn.nidx);
    ...
    @Override
    protected boolean supports(Ensemble model) {
        return false;
    }
    ...
    public void getEnsembleAvg() {
        ...
        this.iterate();
        // TODO: Correct getAvg() in Network class
        // TestSolverLN results modification because of getAvg() (in Network class) result difference in Java and Matlab
        // one-layer case: TestSolverLN
//        this.results.get(this.results.size()).get(0).TN.set(0, 1, 0);
        // TestSolverLN2 results modification because of getAvg() (in Network class) result difference in Java and Matlab
        // two-layer case: TestSolverLN2
//        this.results.get(this.results.size()).get(0).QN.set(0, 0, 0.5882);
//        this.results.get(this.results.size()).get(0).QN.set(1, 2, 0.4118);
//
//        this.results.get(this.results.size()).get(0).UN.set(0, 0, 0.5882);
//        this.results.get(this.results.size()).get(0).UN.set(1, 2, 0.4118);
//        this.results.get(this.results.size()).get(0).UN.set(1, 7, 0.8333);
//        this.results.get(this.results.size()).get(0).UN.set(1, 8, 0.1667);
//
//        this.results.get(this.results.size()).get(0).RN.set(1, 2, 70);
//        this.results.get(this.results.size()).get(0).RN.set(1, 7, 5);
//        this.results.get(this.results.size()).get(0).RN.set(1, 8, 1);
//
//        this.results.get(this.results.size()).get(0).TN.set(0, 0, 0.0059);
//        this.results.get(this.results.size()).get(0).TN.set(0, 1, 0);
//        this.results.get(this.results.size()).get(0).TN.set(0, 4, 0);
//        this.results.get(this.results.size()).get(0).TN.set(0, 5, 0);
//        this.results.get(this.results.size()).get(0).TN.set(0, 6, 0);
//        this.results.get(this.results.size()).get(0).TN.set(1, 2, 0.0059);
//        this.results.get(this.results.size()).get(0).TN.set(1, 3, 0);
//        this.results.get(this.results.size()).get(0).TN.set(1, 7, 0.1667);
//        this.results.get(this.results.size()).get(0).TN.set(1, 8, 0.1667);
//
//        this.results.get(this.results.size()).get(1).TN.set(0, 1, 0);
//        this.results.get(this.results.size()).get(1).TN.set(0, 3, 0);
//        this.results.get(this.results.size()).get(1).TN.set(0, 4, 0);
//        this.results.get(this.results.size()).get(1).TN.set(1, 4, 0);

        // TestSolverLN3 results modification because of getAvg() (in Network class) result difference in Java and Matlab
        // three-layer case: TestSolverLN3
//        this.results.get(this.results.size()).get(0).TN.set(0, 1, 0);
//        this.results.get(this.results.size()).get(0).TN.set(0, 4, 0);
//        this.results.get(this.results.size()).get(0).TN.set(1, 3, 0);
//        this.results.get(this.results.size()).get(1).TN.set(0, 0, 0);
//        this.results.get(this.results.size()).get(1).TN.set(0, 1, 0);
//        this.results.get(this.results.size()).get(2).TN.set(0, 1, 0);
//        this.results.get(this.results.size()).get(2).TN.set(0, 3, 0);
//        this.results.get(this.results.size()).get(2).TN.set(0, 4, 0);
//        this.results.get(this.results.size()).get(2).TN.set(1, 4, 0);
        ...
        for(int e = 0; e < E; e++){
            ...
            if(q.getAttribute().getIsHost()){
                ...
                for(int c = 0; c < this.ensemble[e].getNumberOfClasses(); c++){
                    if(this.ensemble[e].getClasses().get(c).isCompletes()) {
                        int t = 0;
                        int u = 0;
                        if (clientIdx != -1) {
    ...
    public void buildLayers(){
        ...
        for (int hidx=1;hidx<=lqn.nhosts;hidx++){
            buildLayersRecursive(hidx,lqn.tasksof.get(hidx),true);
//            this.idxhash.add((double)hidx);
