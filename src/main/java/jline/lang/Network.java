...
import static jline.api.SN.snHasMultiClassHeterFCFS;
...
public class Network extends Model implements Serializable {
    ...
    public jline.lang.RoutingMatrix serialRouting(List<JobClass> jobClasses, Node... nodes) {
        if (nodes.length == 0) {
            return new jline.lang.RoutingMatrix();
        }

        Network network = nodes[0].model;
        jline.lang.RoutingMatrix outMatrix = new jline.lang.RoutingMatrix(this, jobClasses, network.nodes);
    ...
    public jline.lang.RoutingMatrix serialRouting(JobClass jobClass, Node... nodes) {
    ...
    public jline.lang.RoutingMatrix serialRouting(Node... nodes) {
        if (nodes.length == 0) {
            return new jline.lang.RoutingMatrix();
        }
        Network network = nodes[0].model;
...
//        			nodetypes.add(NodeType.Transition);
    ...
    public routingMatrixReturn getRoutingMatrix(Matrix arvRates, int returnVal) {
        ...
        if (this.csMatrix == null) {
            ...
            Set<Set<Integer>> chainCandidates = UTIL.weaklyConnect(this.csMatrix, new HashSet<Integer>());
                ...
                case Cache:
                    ...
                    for(int r = 0; r < this.getNumberOfClasses(); r++){
                        if(!cache.popularityGet(r).isDisabled()){
                            param.nitems = (int) UTIL.max(param.nitems, cache.popularityGet(r).getSupport().getRight());
                        }
                    }
...
//	    			}
    ...
    public Map<Integer[], Matrix> initRoutingMatrix() {
        Map<Integer[], Matrix> routingMatrix = new HashMap<>();
        int k = getNumberOfNodes();
        int m = getNumberOfClasses();
        for (int i = 1; i <= k; i++) {
            for (int j = 1; j <= k; j++) {
                Integer[] key = {i, j};
                routingMatrix.put(key, new Matrix(m,m,m*m));
            }
        }
