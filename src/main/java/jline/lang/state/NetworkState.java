...
import jline.util.Permutations;
...
public class NetworkState implements Serializable {
  ...
  */
  ...
  public final Map<Station, JLineMatrix> initialState;
  public final Map<Station, JLineMatrix> priorInitialState;

  public NetworkState(
      Map<Station, JLineMatrix> initialState, Map<Station, JLineMatrix> priorInitialState) {
  ...
  public static class StateMarginalStatistics {
    ...
    public JLineMatrix ni;
    public JLineMatrix nir;
    public JLineMatrix sir;
    public List<JLineMatrix> kir;

    public StateMarginalStatistics(
        JLineMatrix ni, JLineMatrix nir, JLineMatrix sir, List<JLineMatrix> kir) {
      this.ni = ni;
      this.nir = nir;
      this.sir = sir;
      this.kir = kir;
  ...
  public static StateMarginalStatistics toMarginal(
      ...
      JLineMatrix state_i,
      JLineMatrix phasesz,
      JLineMatrix phaseshift,
      JLineMatrix space_buf,
      JLineMatrix space_srv,
      JLineMatrix space_var) {
    ...
    if (phasesz == null) {
      phasesz = new JLineMatrix(1, sn.phasessz.getNumCols());
      JLineMatrix.extract(sn.phasessz, ist, ist + 1, 0, sn.phasessz.getNumCols(), phasesz, 0, 0);
    }
    if (phaseshift == null) {
      phaseshift = new JLineMatrix(1, sn.phaseshift.getNumCols());
      JLineMatrix.extract(
    ...
    if (space_var == null) {
      ...
      space_var = new JLineMatrix(state_i.getNumRows(), col);
      JLineMatrix.extract(
          state_i,
          0,
          state_i.getNumRows(),
    ...
    if (space_srv == null) {
      ...
      space_srv = new JLineMatrix(state_i.getNumRows(), sumPhasesz);
      JLineMatrix.extract(
          state_i,
          0,
          state_i.getNumRows(),
    ...
    if (space_buf == null) {
      ...
      space_buf = new JLineMatrix(state_i.getNumRows(), col);
      JLineMatrix.extract(state_i, 0, state_i.getNumRows(), 0, col, space_buf, 0, 0);
    }

    JLineMatrix nir = new JLineMatrix(state_i.getNumRows(), R);
    JLineMatrix sir = new JLineMatrix(state_i.getNumRows(), R);
    List<JLineMatrix> kir = new ArrayList<>();
    if (isExponential) {
      sir = space_srv;
      kir.add(space_srv);
    } else {
      ...
      for (int i = 0; i < phasesz.elementMax(); i++)
        kir.add(new JLineMatrix(state_i.getNumRows(), R));

      for (int r = 0; r < R; r++) {
        for (int k = 0; k < phasesz.get(r); k++) {
          // kir(:,r,k) = space_srv(:,phaseshift(r)+k);
          JLineMatrix tmp_kir = kir.get(k);
          JLineMatrix.extract(
              space_srv,
              0,
    ...
    switch (sn.sched.get(sn.stations.get(ist))) {
      ...
      case LCFSPR:
        if (space_buf.length() > 1) {
          ...
          JLineMatrix sub_space_buf =
              new JLineMatrix(1, (space_buf.getNumCols() * space_buf.getNumRows() + 1) / 2);
          for (int i = 0; i < sub_space_buf.getNumCols(); i++)
            sub_space_buf.set(0, i, space_buf.get(i * 2));

          for (int r = 0; r < R; r++) {
            JLineMatrix sumval = sub_space_buf.countEachRow(r);
            for (int i = 0; i < sir.getNumRows(); i++) nir.set(i, r, sir.get(i, r) + sumval.get(i));
          }
        } else {
          nir = sir.clone();
    ...
    JLineMatrix ni = nir.sumRows();

    return new StateMarginalStatistics(ni, nir, sir, kir);
  }

  public static JLineMatrix fromMarginalAndStarted(
      NetworkStruct sn, int ind, JLineMatrix n, JLineMatrix s) {
    return fromMarginalAndStarted(sn, ind, n, s, true);
  }

  public static JLineMatrix fromMarginalAndStarted(
      Network network, int ind, JLineMatrix n, JLineMatrix s) {
    return fromMarginalAndStarted(network.getStruct(true), ind, n, s, true);
  }

  public static JLineMatrix fromMarginalAndStarted(
      NetworkStruct sn, int ind, JLineMatrix n, JLineMatrix s, Boolean optionsForce) {
    ...
    int R = sn.nClasses;
    JLineMatrix S = sn.nservers;
    int ist = (int) sn.nodeToStation.get(0, ind);

    JLineMatrix K = new JLineMatrix(1, R);
    for (int r = 0; r < R; r++) {
    ...
    JLineMatrix state = new JLineMatrix(0, 0);
    JLineMatrix space = new JLineMatrix(0, 0);
    ...
    if ((sn.nodetypes.get(ind) == NodeType.Queue)
      ...
      switch (sn.sched.get(sn.stations.get(ist))) {
        case EXT:
          for (int r = 0; r < R; r++) {
            JLineMatrix init = NetworkState.spaceClosedSingle(K.get(0, r), 0);
            ...
            state = NetworkState.decorate(state, init);
          }
          space = NetworkState.decorate(space, state);
          // TODO: check here, line 65
          JLineMatrix infinities = new JLineMatrix(space.getNumRows(), 1);
          for (int i = 0; i < infinities.getNumRows(); i++) {
            infinities.set(i, 0, Double.POSITIVE_INFINITY);
          }
          JLineMatrix newSpace =
              new JLineMatrix(
        ...
        case LCFS:
          ...
          JLineMatrix inbuf = new JLineMatrix(0, 0);
          for (int r = 0; r < R; r++) {
            if (n.get(0, r) > 0) {
              JLineMatrix newInbuf =
                  new JLineMatrix(1, inbuf.getNumCols() + (int) n.get(0, r) - (int) s.get(0, r));
          ...
          JLineMatrix mi = Permutations.uniquePerms(inbuf);
          JLineMatrix mi_buf;
          double sumN = n.sumSubMatrix(0, n.getNumRows(), 0, n.getNumCols());
          double sumS = s.sumSubMatrix(0, s.getNumRows(), 0, s.getNumCols());
          if (mi.isEmpty()) {
            mi_buf = new JLineMatrix(1, (int) Math.max(1, sumN - S.get(ist, 0)));
            mi_buf.zero();
            state = new JLineMatrix(1, (int) K.sumSubMatrix(0, K.getNumRows(), 0, K.getNumCols()));
            state.zero();
            JLineMatrix newState = new JLineMatrix(1, mi_buf.getNumCols() + state.getNumCols());
            ...
            if (sumN > sumS) {
              mi_buf = new JLineMatrix(mi.getNumRows(), (int) sumN - (int) sumS);
              ...
              mi_buf = new JLineMatrix(1, 1, 1);
              mi_buf.set(0, 0, 0);
            }
          }

          // mi_srv: class of jobs running in the server of i
          JLineMatrix mi_srv = new JLineMatrix(0, 0);
          for (int r = 0; r < R; r++) {
            JLineMatrix new_mi_srv = new JLineMatrix(1, mi_srv.getNumCols() + (int) s.get(0, r));
          ...
          JLineMatrix si = s.clone();
          for (int b = 0; b < mi_buf.getNumRows(); b++) {
            for (int k = 0; k < si.getNumRows(); k++) {
              JLineMatrix kState = new JLineMatrix(0, 0);
              for (int r = 0; r < R; r++) {
                JLineMatrix init = NetworkState.spaceClosedSingle(K.get(0, r), 0);
                init.set(0, 0, si.get(k, r));
                kState = NetworkState.decorate(kState, init);
              }
      ...
      if (sn.nodetypes.get(ind) == NodeType.Join) {
        space = new JLineMatrix(1, 1);
        space.zero();
      }
    }

    // Required to sort empty state as first
    List<JLineMatrix> uniqueRows = new ArrayList<>();
    for (int i = 0; i < space.getNumRows(); i++) {
      JLineMatrix tmp = new JLineMatrix(1, space.getNumCols());
      JLineMatrix tmp2 = new JLineMatrix(1, space.getNumCols());
      JLineMatrix.extractRows(space, i, i + 1, tmp);
      boolean unique = true;
      for (int j = i + 1; j < space.getNumRows(); j++) {
        JLineMatrix.extractRows(space, j, j + 1, tmp2);
        if (tmp == tmp2) {
          unique = false;
        }
      }
    ...
    JLineMatrix newSpace = new JLineMatrix(uniqueRows.size(), uniqueRows.get(0).getNumCols());
  ...
  public static JLineMatrix decorate(JLineMatrix inSpace1, JLineMatrix inSpace2) {
    ...
    for (int s = 0; s < n2; s++) {
      JLineMatrix tmp = new JLineMatrix(1, inSpace2.getNumCols());
      JLineMatrix.extractRows(inSpace2, s, s + 1, tmp);
  ...
  private static JLineMatrix spaceClosedSingle(double M, double N) {
  ...
  private static JLineMatrix multiChoose(double n, double k) {

    JLineMatrix v = new JLineMatrix(1, (int) n);
    v.zero();

    if (n == 1) {
      v = new JLineMatrix(1, 1);
      v.set(0, 0, k);
    } else if (k != 0) {
      List<JLineMatrix> tmpSSRows = new ArrayList<>();
      for (int i = 0; i <= k; i++) {
        JLineMatrix w = multiChoose(n - 1, k - i);
        JLineMatrix tmpSSRow = new JLineMatrix(w.getNumRows(), w.getNumCols() + 1);
  ...
  public static boolean isValid(Network sn, JLineMatrix n, JLineMatrix s) {
  ...
  public static boolean isValid(NetworkStruct sn, JLineMatrix n, JLineMatrix s) {
  ...
  public static JLineMatrix fromMarginal(NetworkStruct sn, int ind, JLineMatrix n) {
    ...
    if (sn.isstateful.get(ind, 0) == 1 && sn.isstation.get(ind, 0) == 0) {
      for (int r = 0; r < R; r++) {
        JLineMatrix init_r = spaceClosedSingle(1, n.get(r));
        state = decorate(state, init_r);
      }
      return decorate(space, state);
    }

    JLineMatrix phases = new JLineMatrix(1, R);
    for (int r = 0; r < R; r++) {
      if (sn.proc.get(sn.stations.get(ist)).get(sn.jobClasses.get(r)).isEmpty()) {
        phases.set(0, r, 0);
      } else {
      ...
      case Source:
        switch (sn.sched.get(sn.stations.get(ist))) {
          ...
          case GPS:
            // In these policies we only track the jobs in the servers
            for (int r = 0; r < R; r++) {
              JLineMatrix init_r = spaceClosedSingle(phases.get(r), n.get(r));
              state = decorate(state, init_r);
            }
