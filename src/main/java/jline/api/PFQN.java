...
import static jline.api.UTIL.oner;
import static jline.util.Hash.hashpop;
import static jline.util.Sequence.pprod;

/**
* APIs for Product-Form Queueing Networks.
*/    
public class PFQN {

	private static double Fz(Matrix Z, Matrix n) {
		int R = n.length();
	...
	public static pfqnCAReturn pfqn_ca(Matrix L, Matrix N, Matrix Z) {
		int M = L.getNumRows();
		int R = L.getNumCols();
		...
		if (M == 0) {
			...
			return new pfqnCAReturn(Gn, lGn);
		}

		if (N.elementMin() < 0) {
			return new pfqnCAReturn(0, Double.NEGATIVE_INFINITY);
		}

		if (N.sumRows().sumCols().get(0) == 0) {
			return new pfqnCAReturn(1.0, 0.0);
		}
	...
	public static pfqnMmint2GausslegendreReturn pfqn_mmint2_gausslegendre(Matrix L, Matrix N, Matrix Z, Integer m) {
		if (m == null) {
			m = 1;
		}
	...
	private static Matrix pfqn_le_hessianZ(Matrix L, Matrix N, Matrix Z,Matrix u,double v) {
		int K = L.getNumRows();
		int R = L.getNumCols();
	...
	private static Matrix pfqn_le_hessian(Matrix L, Matrix N, Matrix u0) {
		int M = L.numRows;
		int R = L.numCols;
		double Ntot = N.elementSum();
	...
	private static pfqnLeFpiReturn pfqn_le_fpi(Matrix L, Matrix N) {
		int M = L.numRows;
		int R = L.numCols;
	...
	private static pfqnLeFpiZReturn pfqn_le_fpiZ(Matrix L, Matrix N, Matrix Z) {
		int M = L.numRows;
		int R = L.numCols;
	...
	private static double simplex_fun(double[] x, Matrix L, Matrix N) {
		int M = x.length + 1;
	...
	public static pfqnLeReturn pfqn_le(Matrix L, Matrix N, Matrix Z) {
		int M = L.numRows;
		int R = L.numCols;
		double lGn;
		double Gn;

		if (L.isEmpty() || N.isEmpty() || N.elementSum() == 0 || L.elementSum() < 1e-4) {
		...
		return new pfqnLeReturn(Gn, lGn);
	}


	public static pfqnLsReturn pfqn_ls(Matrix L, Matrix N, Matrix Z, int I) {
		int M = L.numRows;
		int R = L.numCols;
		...
		if (L_new.isEmpty() || N.isEmpty() || N.elementSum() == 0 || L_new.elementSum() < 1e-4) {
			Matrix tmp = new Matrix(1, Z.numCols);
			for (int i = 0; i < tmp.length(); i++) {
				tmp.set(i, Math.log(Z.sumCols(i)));
			}
	...
	private static double pfqn_ls_helper(double[] x, int K, int M, double eta, double eN, Matrix L, Matrix N, Matrix Z) {
	...
	public static pfqnNcSanitizeReturn pfqn_nc_sanitize(Matrix lambda, Matrix L, Matrix N, Matrix Z, double atol) {
		...
		for (int i = 0; i < N.length(); i++) {
			if (Math.abs(N.get(i)) >= 1e-6 && !(L_new.sumCols(i)+Z_new.sumCols(i) < atol)) {
	...
	public static computeNormConstReturn compute_norm_const(Matrix L, Matrix N, Matrix Z, SolverOptions options) {
		int M = L.numRows;
		int R = L.numCols;
		...
		switch (options.method) {
			case "ca": {
				pfqnCAReturn ret = pfqn_ca(L,N,Z.sumCols());
				lG = ret.lGn;
				break;
			}
			case "adaptive":
			case "default": {
				...
				if (M > 1) {
					if (R == 1 || (R <= 3 && N.elementSum() < 50)) {
						...
						if (M <= R) {
							pfqnLeReturn ret = pfqn_le(L,N,Z.sumCols());
							lG = ret.lGn;
							method = "le";
						}
					}
				} else if (Z_colSum.numCols == 1 && Math.abs(Z_colSum.get(0)) < 1e-6) {
					Matrix tmp = L.clone();
					for (int i = 0; i < tmp.length(); i++) {
						tmp.set(i, Math.log(tmp.get(i)));
					}
					...
					if (N.elementMax() < 10000) {
						pfqnMmint2GausslegendreReturn ret = pfqn_mmint2_gausslegendre(L,N,Z.sumCols(),null);
						lG = ret.lG;
						method = "gleint";
					} else {
						pfqnLeReturn ret = pfqn_le(L,N,Z.sumCols());
			...
			case "sampling": {
				pfqnLsReturn ret = pfqn_ls(L,N,Z.sumCols(),options.samples);
				lG = ret.lGn;
				method = "ls";
				break;
			}
		...
		return new computeNormConstReturn(lG, X, Q, method);
	}

  // TODO: Change the option parameter to varargin once Solver.parseOptions() is implemented.
	public static pfqnNcReturn pfqn_nc(Matrix lambda, Matrix L, Matrix N, Matrix Z, SolverOptions options) {
    /*
     ...
     */
		...
		if (N_new.elementSum() < 1e-6 || N_new.isEmpty()) {
			lG = 0.0;
    ...
    computeNormConstReturn ret = compute_norm_const(L_new, N_new, Z_new, options);
	...
	public static pfqnNcldReturn pfqn_ncld(Matrix L, Matrix N, Matrix Z, Matrix mu, SolverOptions options) {
		...
		if (N_new.elementMin() < 0.0) {
			...
			computeNormConstLDReturn ret = compute_norm_const_ld(L_new, N_new, Z_new, mu_new, options);
			lGnnzdem = ret.lG;
			method = ret.method;
		}
	...
	//TODO: other case to be implemented
	public static computeNormConstLDReturn compute_norm_const_ld(Matrix L, Matrix N, Matrix Z, Matrix mu, SolverOptions options) {
		int M = L.numRows;
		int R = L.numCols;
		String method = options.method;
		Double lG = null;
		...
		switch (options.method) {
			...
			case "exact": {
				...
				} else if (!(M == 1 && Z.elementMax() > 1e-6)) {
					pfqnGldReturn ret = pfqn_gld(Lz, N, muz, options);
					lG = ret.lG;
				}
				method = "exact";
				break;
		...
		return new computeNormConstLDReturn(lG, method);
	}

	public static pfqnGldReturn pfqn_gld(Matrix L, Matrix N, Matrix mu, SolverOptions options) {
		int M = L.numRows;
		int R = L.numCols;
		...
		if (R == 1) {
			pfqnGldSingleReturn ret = pfqn_gldsingle(L, N, mu, null);
			lG = ret.lG;
			G = ret.G;
			return new pfqnGldReturn(G, lG);
		}
		...
		for (int i = 0; i < M; i++) {
			...
			for (int j = 0; j < mu_row_i.numCols; j++) {
				if (Math.abs(mu_row_i.get(j)-(j+1))>1e-6) {
					flag = false;
					break;
				}
			}

			if (Math.abs(mu_row_i.elementMin()-1)<1e-6 && Math.abs(mu_row_i.elementMax()-1)<1e-6) {
				isInfServer[i] = false;
			} else if (flag) {
				isInfServer[i] = true;
			} else {
		...
		if (Math.abs(N.elementMax())<1e-6 && Math.abs(N.elementMin())<1e-6) {
			G = 1.0;
			lG = Math.log(G);
			return new pfqnGldReturn(G, lG);
		}
	...
	public static pfqnGldSingleReturn pfqn_gldsingle(Matrix L, Matrix N, Matrix mu, SolverOptions options) {
		int M = L.numRows;
		int R = L.numCols;
		...
		Map<Index, Double> g = new HashMap<>();
		g.put(new Index(1, 1, 1), 0.0);
		for (int n = 1; n <= N.get(0); n++) {
			g.put(new Index(1,n+1,2), 0.0);
		}

		for (int m = 1; m <= M; m++) {
			for (int tm = 1; tm <= N.get(0)+1; tm++) {
				g.put(new Index(m+1, 1, tm+1), 1.0);
			}
			for (int n = 1; n <= N.get(0); n++) {
				for (int tm = 1; tm <= N.get(0)-n+1; tm++) {
					g.put(new Index(m+1,n+1,tm+1),
									g.get(new Index(m,n+1,2))
													+ L.get(m-1)*g.get(new Index(m+1,n,tm+2))/mu.get(m-1,tm-1));
				}
			}
		}
		double G = g.get(new Index(M+1, (int)N.get(0)+1, 2));
		double lG = Math.log(G);
		return new pfqnGldSingleReturn(G, lG);
	}
	...
	public static double pfqn_qzgbup(Matrix L, double N, double Z, int i){
		...
		double Yi = L.get(i) * UTIL.min(1.0/L.elementMax(), N/(Z + sumL + sigma * (N - 1 - Z * pfqn_xzabaup(L, N-1, Z))));
		double Qgb;
		if(Yi < 1){
	 ...
	 */
	public static pfqnMVAMSReturn pfqn_mvams(Matrix lambda, Matrix L, Matrix N, Matrix Z, Matrix mi, Matrix S){
		int M = L.getNumRows();
		int R = L.getNumCols();
		double Ntot = 0;
		...
		for(int i = 0; i < M; i++){
			for(int j = 0; j < Ntot; j++){
				mu.set(i, j, UTIL.min(j+1, S.get(i)));
			}
		}
		double maxS = 0;
		...
		pfqnMVAMSReturn returnObject;
		if(maxS == 1){
			// No multi-server nodes
			if(NhasInf){
				// open or mixed model
				pfqnMVAMXReturn retMVAMX = pfqn_mvamx(lambda, L, N, Z, mi);
				returnObject = new pfqnMVAMSReturn(retMVAMX.XN, retMVAMX.QN, retMVAMX.UN, retMVAMX.CN, retMVAMX.lGN);
			} else {
				// closed model
				pfqnMVAReturn retMVA = pfqn_mva(L,N,Z,mi);
				returnObject = new pfqnMVAMSReturn(retMVA.XN, retMVA.QN, retMVA.UN, retMVA.CN, retMVA.lGN);
			}
		} else {
			// Multi-server nodes
			if(NhasInf){
				...
				if(mi.elementMax() == 1){
					double lG = Double.NaN;
					pfqnMVALDMSReturn retMVALDMS = pfqn_mvaldms(lambda, L, N, Z, S);
					returnObject = new pfqnMVAMSReturn(retMVALDMS.XN, retMVALDMS.QN, retMVALDMS.UN, retMVALDMS.CN, lG);
				} else {
	 ...
	 */
	public static pfqnMVAMXReturn pfqn_mvamx(Matrix lambda, Matrix D, Matrix N, Matrix Z, Matrix mi){
		...
		return new pfqnMVAMXReturn(XN, QN, UN, CN, ret1.lGN);
	}
	 ...
	 * @return - the performance measures for the given network.
	 */
	public static pfqnMVALDMSReturn pfqn_mvaldms(Matrix lambda, Matrix D, Matrix N, Matrix Z, Matrix S){
		int M = D.getNumRows();
		int R = D.getNumCols();
		double Nct = 0;
		for(int i = 0; i < N.getNumRows(); i++){
		...
		return new pfqnMVALDMSReturn(XN, QN, UN, CN, lGN);
	}
		...
		* */
		...
		while(!(nvec.getNumRows() == 1 && nvec.getNumCols() == 1 && nvec.get(0,0) == -1)){
			...
			for(int i = 0; i < M; i++){
				...
				Pc[i].set(0, hnvec, UTIL.max(Math.ulp(1.0), 1 - sumpc));
			}
	...
	public static pfqnMVALDMXECReturn pfqn_mvaldmx_ec(Matrix lambda, Matrix D, Matrix mu){
		int M = mu.getNumRows();
		int R = mu.getNumCols();
	...
	public static pfqnLinearizerMSReturn pfqn_linearizerms(Matrix L, Matrix N, Matrix Z, Matrix nservers, Matrix type){
		return pfqn_linearizerms(L, N, Z, nservers, type, 1e-8);
	}
	...
	private static pfqnLinearizerMSCoreReturn linearizerms_core(Matrix L, int M, int R, Matrix N_1, Matrix Z, Matrix nservers,
	...
	private static pfqnLinearizerMSEstimateReturn linearizerms_estimate(int M, int R, Matrix N_1, Matrix nservers, Matrix Q, Matrix P, Matrix PB, Matrix[] Delta) {
	...
	private static pfqnLinearizerMSForwardMVAReturn linearizerms_forwardMVA(Matrix L, int M, int R, Matrix N_1, Matrix Z,
	...
	/**
	 * Extended general form linearizer approximation algorithm
	 * @param L - the service demand matrix
	 * @param N - the population vector
	...
	public static pfqnLinearizerCoreReturn egflinearizer_core(Matrix L, int M, int R, Matrix N_1, Matrix Z, Matrix Q,
	...
	public static pfqnLinearizerEstimateReturn egflinearizer_estimate(Matrix L, int M, int R, Matrix N_1, Matrix Z,
	...
	public static pfqnLinearizerForwardMVAReturn egflinearizer_forwardMVA(Matrix L, int M, int R, SchedStrategy[] type,
	...
	public static class pfqnMVAMSReturn {
		public Matrix XN;
		public Matrix QN;
		public Matrix UN;
		public Matrix CN;
		...
		public pfqnMVAMSReturn(Matrix XN, Matrix QN, Matrix UN, Matrix CN, double lG) {
			this.XN = XN;
			this.QN = QN;
			this.UN = UN;
			this.CN = CN;
	...
	public static class pfqnMVAMXReturn {
		public Matrix XN;
		public Matrix QN;
		public Matrix UN;
		public Matrix CN;
		...
		public pfqnMVAMXReturn(Matrix XN, Matrix QN, Matrix UN, Matrix CN, double lGN) {
			this.XN = XN;
			this.QN = QN;
			this.UN = UN;
			this.CN = CN;
	...
	public static class pfqnMVALDMSReturn {
		public Matrix XN;
		public Matrix QN;
		public Matrix UN;
		public Matrix CN;
		...
		public pfqnMVALDMSReturn(Matrix XN, Matrix QN, Matrix UN, Matrix CN, double lGN) {
			this.XN = XN;
			this.QN = QN;
			this.UN = UN;
			this.CN = CN;
	...
	public static class pfqnCAReturn {
		public double Gn;
		public double lGn;

		public pfqnCAReturn(double Gn, double lGn) {
			this.Gn = Gn;
			this.lGn = lGn;
		}
	}

	public static class pfqnMmint2GausslegendreReturn {
		public double G;
		public double lG;

		public pfqnMmint2GausslegendreReturn(double G, double lG) {
			this.G = G;
			this.lG = lG;
		}
	}
	...
	public static class pfqnLeReturn {
		public double Gn;
		public double lGn;

		public pfqnLeReturn(double Gn, double lGn) {
			this.Gn = Gn;
			this.lGn = lGn;
		}
	}

	public static class pfqnLsReturn {
		public double Gn;
		public double lGn;

		public pfqnLsReturn(double Gn, double lGn) {
			this.Gn = Gn;
			this.lGn = lGn;
		}
	}
	...
	public static class pfqnNcReturn {
		...
		public pfqnNcReturn(Double lG, Matrix X, Matrix Q, String method) {
			this.lG = lG;
			this.X = X;
			this.Q = Q;
			this.method = method;
	...
	public static class computeNormConstReturn {
		public Double lG;
		public Matrix X;
		public Matrix Q;
		public String method;

		public computeNormConstReturn(Double lG, Matrix X, Matrix Q, String method) {
			this.lG = lG;
			this.X = X;
			this.Q = Q;
			this.method = method;
	...
	public static class pfqnNcldReturn {
		public Double lG;
		public Double G;
		public String method;

		public pfqnNcldReturn(Double lG, Double G, String method) {
			this.lG = lG;
			this.G = G;
			this.method = method;
		}
	...
	public static class computeNormConstLDReturn {
		public Double lG;
		public String method;

		public computeNormConstLDReturn(Double lG, String method) {
			this.lG = lG;
			this.method = method;
		}
	}

	public static class pfqnGldReturn {
		public Double G;
		public Double lG;

		public pfqnGldReturn(Double G, Double lG) {
			this.G = G;
			this.lG = lG;
		}
	}

	public static class pfqnGldSingleReturn {
		public Double G;
		public Double lG;

		public pfqnGldSingleReturn(Double G, Double lG) {
			this.G = G;
			this.lG = lG;
		}
	}
