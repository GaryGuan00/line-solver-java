	...
	public SolverResult run() {
		...
		while(forkLoop && forkIter < this.options.iter_max){
			...
			if(this.options.method.equals("exact") && !this.solver.model.hasProductFormSolution()){
				throw new RuntimeException("The exact method requires the model to have a product-form solution. This model does not have one.");
			}
			if(this.options.method.equals("mva") && !this.solver.model.hasProductFormSolution()){
				System.err.println("The exact method requires the model to have a product-form solution. This model does not have one. SolverMVA will return an approximation generated by an exact MVA algorithm.");
			}
						...
						case "fjt": case "default":
							...
							for(int s : forkauxclasses){
								...
								for(int pow = 0; pow < parallel_branches; pow++){
									Matrix nk = UTIL.nchoosek(lambdai, pow + 1);
									nk = nk.sumRows();
