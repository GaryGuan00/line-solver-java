        ...
        if(SN.snHasProductFormExceptMultiClassHeterExpFCFS(sn) && !SN.snHasLoadDependence(sn) &&
            ...
            switch(options.method){
                ...
                case "bs":
                    ...
                    PFQN.pfqnBSReturn bsret = PFQN.pfqn_bs(L, N, Z, options.tol, options.iter_max, null, schd);
                    X = bsret.XN;
                    int iResult = 0;
                    for(int i : queueIdx){
                        for(int j = 0; j < C; j++){
                    ...
                    if(nservers.elementMax() == 1){
                        ...
                        switch(options.config.multiserver){
                            ...
                            case "krzesinski":
                                ...
                                PFQN.pfqnLinearizerReturn res = pfqn_linearizermx(lambda, L, N, Z, nservers, schdi, options.tol, options.iter_max, null);
                                int iRes = 0;
                                for(int i : queueIdx){
                                    for(int j = 0; j < C; j++){
                                        Q.set((int) sn.nodeToStation.get(i), j, res.Q.get(iRes, j));
