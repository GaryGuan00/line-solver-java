...
public class SIROClassBuffer extends StateCell {
    ...
    private void moveToService() {
        Cdf<Integer> nextClassCdf = new Cdf<Integer>(this.random);

        int totalInWaiting = 0;
        for (int i = 0; i < this.nClasses; i++) {
            nextClassCdf.addElement(i, this.inWaiting[i]);
        ...
        nextClassCdf.normalize(totalInWaiting);


        int classToAdd = nextClassCdf.generate();
        this.phaseList.addToService(classToAdd);
        this.inService[classToAdd] += 1;
        this.inWaiting[classToAdd] -= 1;
    }

    public SIROClassBuffer(Random random, int nClasses, int nServers, PhaseList phaseList, boolean isPreemptive) {
    ...
    public SIROClassBuffer(Random random, int nClasses, int nServers, PhaseList phaseList) {
    ...
    public StateCell createCopy() {
        SIROClassBuffer copyBuffer = new SIROClassBuffer(this.random, this.nClasses, this.nServers, this.phaseList.createCopy());
