...
public class SN {
    ...
    public static void snRefreshVisits(NetworkStruct sn, Matrix chains, Matrix rt, Matrix rtnodes) {
    	int I = sn.nnodes;
    	int M = sn.nstateful;
    	int K = sn.nclasses;
	...
	public static snGetProductFormChainParamsReturn snGetProductFormParams(NetworkStruct sn){
		...
		for(int i = 0; i < Mq; i++){
			...
			for(int j = 0; j < mu.getNumCols(); j++){
				mu.set(i, j, UTIL.min(j + 1, sn.nservers.get((int) sn.nodeToStation.get(queueIndices.get(i)))));
			}
		}
		Matrix Z = new Matrix((int) UTIL.max(1, Mz), R);
		for(int i = 0; i < Mz; i++){
			for(int r = 0; r < R; r++){
				int c = 0;
				for(; c < sn.chains.getNumRows(); c++){
