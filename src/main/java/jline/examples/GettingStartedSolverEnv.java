...
import jline.util.CirculantMatrix;
import org.apache.commons.math3.optim.PointValuePair;

import java.util.Arrays;

public class GettingStartedSolverEnv {

  public static void main(String[] args) {

    SolverEnv envSolver = exampleRandomEnvironment1();
  ...
  public static SolverEnv exampleRandomEnvironment1() {
    ...
    JLineMatrix rate = new JLineMatrix(M, E);
    rate.set(0, 0, 2);
    rate.set(0, 1, 1);
    rate.set(1, 0, 1);
    rate.set(1, 1, 2);
    ...
    envSubModel[0] = exampleRandomEnvironmentGenModel(JLineMatrix.extractColumn(rate, 0, null), N);
    envSubModel[1] = exampleRandomEnvironmentGenModel(JLineMatrix.extractColumn(rate, 1, null), N);

    for (int e = 0; e < E; e++) {
      envModel.addStage(e, envName[e], envType[e], envSubModel[e]);
    }

    JLineMatrix envRates = new JLineMatrix(2, 2);
    envRates.set(0, 0, 0);
    envRates.set(0, 1, 1);
  ...
  public static SolverEnv exampleRandomEnvironment2() {
    ...
    envSubModel[0] = exampleRandomEnvironmentGenModel(JLineMatrix.extractColumn(rate, 0, null), N);
    envSubModel[1] = exampleRandomEnvironmentGenModel(JLineMatrix.extractColumn(rate, 1, null), N);
    envSubModel[2] = exampleRandomEnvironmentGenModel(JLineMatrix.extractColumn(rate, 2, null), N);
    envSubModel[3] = exampleRandomEnvironmentGenModel(JLineMatrix.extractColumn(rate, 3, null), N);

    for (int e = 0; e < E; e++) {
      envModel.addStage(e, envName[e], envType[e], envSubModel[e]);
    }

    JLineMatrix envRates = new JLineMatrix(4, 4);
    envRates.set(0, 0, 0);
  ...
  public static SolverEnv exampleRandomEnvironment3() {
    ...
    JLineMatrix envRates = CirculantMatrix.circul(3);
  ...
  public static SolverEnv exampleRandomEnvironment4() {
    ...
    Env.ResetEnvRatesFunction resetEnvRatesFunction =
        (originalDist, QExit, UExit, TExit) -> {
          ...
          return new Exp(Math.max(lambda, Distribution.zeroRn));
        };
  ...
  public static SolverEnv exampleRandomEnvironment5() {
    ...
    for (int e = 0; e < E; e++) {
      ...
      JLineMatrix targetQueueLengths = searcher.generateTargetQueueLengths(solvers[e].model);
  ...
  private static Network exampleRandomEnvironmentGenModel(JLineMatrix rate, int N) {
    ...
    RoutingMatrix routingMatrix =
        new RoutingMatrix(model, Arrays.asList(cClass), Arrays.asList(delay, queue));
    int numNodes = model.getNumberOfNodes();
    JLineMatrix circulantMatrix = CirculantMatrix.circul(numNodes);
    for (int row = 0; row < numNodes; row++) {
      for (int col = 0; col < numNodes; col++) {
        if (circulantMatrix.get(row, col) == 1) {
          routingMatrix.addConnection(model.getNodes().get(row), model.getNodes().get(col));
        }
      }
    }
